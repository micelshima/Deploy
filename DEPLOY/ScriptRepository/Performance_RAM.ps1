...
if (!(test-path $logfile)){out-file $logfile -input "Computername	PercentUsed	AvailableMB"}
try{
	...
	Append-Richtextbox -ComputerName $computername -Source $scriptbasename -Message 'RAM retrieved successfully' -MessageColor 'blue' -logfile 'ps1command.log'
	$line='{0}	{1:P}	{2}' -f $computername,($UsedMB/$totalMB),$FreeMB
