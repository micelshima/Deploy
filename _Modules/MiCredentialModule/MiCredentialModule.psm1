...
[cmdletBinding()]
	...
	if (!(test-path -path $credfile))
		{
		if (!(test-path -path "$pwd\creds")){new-item -itemtype directory -path "$pwd\creds"}
		...
		$creds.password=$objcreds.GetNetworkCredential().Password|ConvertTo-SecureString -AsPlainText -Force|convertfrom-securestring #DPAPI encryption
...
if(test-path "$pwd\creds"){$array=gci ("$pwd\creds\$username" + "_*.cred")|select @{l='basename';e={$_.basename -replace("$($username)_")}}|select -expand basename}
